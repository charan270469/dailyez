...
3. Matching (shared Gmail + WhatsApp pipeline, `server/agents/signalMatching.js` → `server/agents/orchestrator.js`):
   ...
   - PIPELINE 2 per signal: `signalMessageMatches` routes every message × signal pair through `orchestrateMatch` (`server/agents/orchestrator.js`) — the single matching entry point for Gmail sync, WhatsApp sync, and every re-check:
     - alert-target signals (one-click button or the Add/Edit form's "Alert me" section) → `matchAlertTarget` (deterministic): exact sender email for Gmail; for WhatsApp the target is compared through one canonical JID comparison (chat id / group JID / participant JID via `normalizeAlertTarget`), with the resolved contact/group display name consulted only for name-like targets. Platform-guarded via the normalized message's `source`. No LLM call, and any alert-flavored signal (has `alertTarget` or an explicit `alertEnabled` flag) never falls through to the intent matcher — disabled alerts are fully silent while OFF.
     ...
     - everything else (topic/event/mixed) → `runClassificationPipeline`: keyword pre-filter (skips all LLM work on unrelated messages) → **best-effort fact extraction** (`extractMessageFacts` in `server/agents/extractionAgent.js`, Groq `llama-3.1-8b-instant`, ~300-token JSON: sender name/domain, dates, amounts, named entities, one-line factual summary; any failure is logged and skipped — extraction is an enhancement, never a hard dependency) → shared Groq rolling-window RPM limiter → `checkSignalMatch` (the matchSignal.js LLM matcher, which now receives BOTH the raw message AND the extracted facts as supplementary structured context placed above the raw message content) → **critique verification only when confidence is medium/low** (`verifyMatch` in `server/agents/verificationAgent.js`, Groq `llama-3.1-8b-instant`, ~200-token JSON): the verification agent is handed the signal context, the message, and the initial verdict + reasoning as a CRITIQUE (not a re-run) aimed at the documented false-positive pattern (thematic/superficial similarity — e.g. shared words like "tech" — mistaken for a genuine match); its `finalMatched` overrides the initial result and each stored match carries `verificationRan: true` + `verificationReasoning` for the future "Why this matched" layer. High-confidence classification results skip verification entirely — zero extra Groq call.
   - Each evaluation logs one `[pipeline] path=…` line recording which route ran: `alert-target (deterministic)`, `source-intent (deterministic)`, `classification-pipeline (LLM)`, the pre-filter-skip variant, or the `classification-pipeline (deferred: groq daily budget)` variant. The deferred variant comes from the shared daily Groq budget guard (`server/agents/groqBudget.js`, in-memory per-model counters resetting at local midnight, limits via `GROQ_DAILY_LIMIT_<MODEL>` default 1000, one 80% warning per model per day): every Groq call site records itself via `noteGroqCall(model)` (matching pipeline + voice/summarize/answer), the orchestrator skips LLM evaluation instead of 429-retrying once the extract/match model is exhausted (verification exhaustion only skips the optional critique), deferred pairs stay out of `lastEvaluatedSignalIds` so the next cycle retries them after the reset, and usage is visible at `GET /api/system/groq-budget`.
