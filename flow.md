...
3. Matching (shared Gmail + WhatsApp pipeline, `server/agents/signalMatching.js`):
   ...
   - PIPELINE 1.5 per signal: alert-target signals (`alertEnabled` + `alertTarget`, created by the one-click button) match deterministically in `matchAlertTarget` (`server/agents/signalMatching.js`) — exact sender email for Gmail, exact chat id (bare number / `@g.us`) for WhatsApp, platform-guarded via the normalized message's `source`. No LLM call, and alert signals never fall through to the intent matcher.
