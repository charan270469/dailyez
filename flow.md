...
3. Matching (shared Gmail + WhatsApp pipeline, `server/agents/signalMatching.js`):
   ...
   - PIPELINE 1.5 per signal: alert-target signals (created by the one-click button OR the Add/Edit form's "Alert me" section) match deterministically in `matchAlertTarget` (`server/agents/signalMatching.js`) — exact sender email for Gmail; for WhatsApp the target is compared against EVERY sender identity on the stored message (canonical chat id/phone number, group JID, participant JID, and resolved contact/group display name). Platform-guarded via the normalized message's `source`. No LLM call, and any alert-flavored signal (has `alertTarget` or an explicit `alertEnabled` flag) never falls through to the intent matcher — disabled alerts are fully silent while OFF.
